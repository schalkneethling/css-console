...
- The move is also what makes the bundling happen. Externalization in this toolchain is driven by the production dependency set, not by a bundler default: `getProductionDeps` in `node_modules/.pnpm/@voidzero-dev+vite-plus-core@0.2.6_*/node_modules/@voidzero-dev/vite-plus-core/dist/tsdown/build-oq13wECG-Bymq3jT5.js` line 301 reads `dependencies`, `peerDependencies`, `peerDependenciesMeta`, and `optionalDependencies`, and `externalStrategy` in the same file externalizes an identifier only when that set contains it. A `devDependency` is bundled by that rule alone. `pack.deps.onlyBundle` then states the intent explicitly and turns anything else reaching the bundle into a build error rather than an informational hint.
